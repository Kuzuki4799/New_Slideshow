...
import com.hope_studio.video_maker_slideshow_maker.ho_gs_effect.GSEffect
...
class ConvertVideoToMp4(val gsEffect: GSEffect= GSEffect(), val videoOutWidth:Int, val videoOutHeight:Int, val srcPath:String, val updateProgress:(Long)->Unit) {
    ...
    private fun prepare(outPath: String){
        ...
        mTextureRenderer = VideoEffectTextureRender(RawResourceReader.readTextFileFromRawResource(VideoMakerApplication.getContext(), gsEffect.gsEffectCodeId))
                ...
                if(!mAllInputDecoded) {
                    ...
                    if(bufferId >= 0) {
                        ...
                        if(render) {
                            ...
                            mTextureRenderer!!.draw(mViewPortSize.width, mViewPortSize.height ,mViewPortOffsetX, mViewPortOffsetY,texMatrix, getMvp())

                            EGLExt.eglPresentationTimeANDROID(eglDisplay, eglSurface, mBufferInfo.presentationTimeUs * 1000)

                            EGL14.eglSwapBuffers(eglDisplay, eglSurface)
                            updateProgress.invoke(mBufferInfo.presentationTimeUs/1000-mTimeConvertedMs)
