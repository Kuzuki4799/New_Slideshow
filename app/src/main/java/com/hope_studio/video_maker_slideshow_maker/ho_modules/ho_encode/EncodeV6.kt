package com.hope_studio.video_maker_slideshow_maker.ho_modules.ho_encode
...
import com.hope_studio.video_maker_slideshow_maker.ho_data.RenderImageSlideData
...
class EncodeV6(val renderData: RenderImageSlideData, val part:Int) {
    ...
    fun performEncodeVideo(onUpdateProgress: (Float) -> Unit) {
        try {
            ...
            for (time in 0 until slideShowForRender.totalDuration step 30) {
                ...
                Logger.e("$time/${slideShowForRender.totalDuration}")
            }
            drainEncoder(true)
    ...
    fun performEncodeVideo(startTime:Int, endTime:Int, onEnd:()->Unit) {
        try {
            val start = System.currentTimeMillis()
            prepareEncode()
            mInputSurface.makeCurrent()
            slideDrawer.prepare(renderData.gsTransition)
            for (time in startTime..endTime step 30) {
                drainEncoder(false)
                slideDrawer.changeFrameData(slideShowForRender.getFrameByVideoTimeForRender(time))
                slideDrawer.drawFrame()
                mInputSurface.setPresentationTime((time-(part-1)*5010) * 1000000L)
                mInputSurface.swapBuffer()
    ...
    private fun prepareEncode() {
        ...
        mEncoder.start()
        val outputFilePath = File(OUTPUT_DIR, "encode-$mWidth x $mHeight.$part.mp4").absolutePath
        try {
    ...
    fun drainEncoder(enOfStream: Boolean) {
        ...
        while (true) {
            Logger.e("encoderStatus")
            val encoderStatus = mEncoder.dequeueOutputBuffer(mBufferInfo, timeOutUSec)
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                Logger.e("MediaCodec.INFO_TRY_AGAIN_LATER")
                if (!enOfStream) {
                    Logger.e("!enOfStream")
                    break
                }
            } else if (encoderStatus == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                Logger.e("MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED")
                encoderOutputBuffers = mEncoder.outputBuffers
            } else if (encoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                Logger.e("MediaCodec.INFO_OUTPUT_FORMAT_CHANGED")
                if (mMuxerStarted) {
                ...
                if ((mBufferInfo.flags and MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                    mBufferInfo.size = 0
                    Logger.e("MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0")
                }
                ...
                if (mBufferInfo.size != 0) {
                    ...
                    Logger.e("mBufferInfo.size != 0")
                    encodedData.position(mBufferInfo.offset)
                    encodedData.limit(mBufferInfo.offset + mBufferInfo.size)

                    mMuxer.writeSampleData(mTrackIndex, encodedData, mBufferInfo)
                }
                Logger.e("releaseOutputBuffer")
