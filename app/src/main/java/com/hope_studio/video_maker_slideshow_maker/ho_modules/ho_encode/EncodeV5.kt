...
import com.hope_studio.video_maker_slideshow_maker.ho_data.StickerDrawerData
...
import com.hope_studio.video_maker_slideshow_maker.ho_package_2.ho_slide_show_gl_view_2.StickerDrawer
...
class EncodeV5(val renderData: RenderImageSlideData) {
    ...
    fun performEncodeVideo(onUpdateProgress: (Float) -> Unit, onComplete:()->Unit) {
        ...
        try {
            ...
            for(item in renderData.stickerAddedForRender) {
                ...
                stickerDrawerList.add(stickerDrawerData)
            }
            ...
            for (time in 0 until slideShowForRender.totalDuration step 30) {
                ...
                if(renderData.stickerAddedForRender.size > 0) {
                    for(item in stickerDrawerList) {
                        if(time >= item.startOffset && time <= item.endOffset) {
                            item.stickerDrawer.drawFrame()
                        }
                    }
                }
                ...
                onUpdateProgress.invoke((time*0.9f / slideShowForRender.totalDuration))
    ...
    private fun addMusic(onComplete:()->Unit) {
        if(renderData.musicReturnData != null) {
            FFmpeg(FFmpegCmd.mergeAudioToVideo(renderData.musicReturnData.outFilePath, outputFilePath, FileUtils.getOutputVideoPath(mWidth))).apply {
                runCmd(onComplete)
            }
        } else {
            onComplete.invoke()
