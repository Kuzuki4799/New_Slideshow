...
vec4 effect() {
    ...
    vec2 light1 = vec2(sin(_motion*1.2+45.0)*1.0 + cos(_motion*0.4+32.0)*0.6, sin(_motion*1.2+99.0)*1.2 + cos(_motion*0.2-15.0)*-0.4)*0.25+center;
    vec3 lightColor1 = vec3(1.0, 0.25, 0.25);
    vec2 light2 = vec2(sin(_motion+2.0)*-1.7, cos(_motion+8.0)*1.0)*0.25+center;
    vec3 lightColor2 = vec3(0.25, 1.0, 0.25);
    vec2 light3 = vec2(sin(_motion+3.0)*1.6, cos(_motion+14.0)*-1.2)*0.25+center;
